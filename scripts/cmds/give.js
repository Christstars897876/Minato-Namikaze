...
async function parseAmountWithAPI(input) {
    ...
    // Fallback local avec BigInt pour gérer les grands nombres
...
async function generateTransferImage(senderName, receiverName, amount, icon, senderAvatarUrl, targetAvatarUrl) {
    ...
    // Dessiner avatar de l'expéditeur (gauche)
    await drawAvatar(180, 200, 65, senderAvatarUrl);
    
    // Dessiner avatar du destinataire (droite)
    ...
    // Flèche élégante entre les avatars
    ctx.beginPath();
    ctx.moveTo(280, 200);
    ctx.lineTo(620, 200);
    ctx.strokeStyle = "#d4af37";
    ...
    // Icône décorative sous le montant
